...
function Lesson({ items, weekday, group, onItemClick, lessonNumber }) {
  ...
  return (
    <div className={'row border m-1 p-0 ' + cn()}>
      {items.length === 1 && items[0].weekNumber === 0 && <div className={'p-0' + (items[0].errors ? ' error' : '')}><LessonContent item={items[0]} onItemClick={onItemClick} /></div>}
      {items.length === 2 &&
        <>
          <div className={'border h-50 p-0' + (items[0].errors ? ' error' : '')}>
            <LessonContent item={items[0]} onItemClick={(item) => onItemClick(item, true)} />
          </div>
          <div className={'border h-50 p-0' + (items[1].errors ? ' error' : '')}>
      ...
      {items.length === 1 && items[0].weekNumber === 1 &&
        <>
          <div className='border h-50 p-0' onClick={(e) => onItemClick({ lessonNumber: lessonNumber, weekday: weekday, group: group, weekNumber: 0 }, true)}></div>
          <div className={'border h-50 p-0' + (items[0].errors ? ' error' : '')}>
            <LessonContent item={items[0]} onItemClick={(item) => onItemClick(item, true)} />
          </div>
        </>
