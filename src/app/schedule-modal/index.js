import { memo, useCallback, useState, createRef, useEffect } from "react";
...
function ScheduleModal({ props }) {
  ...
  const callbacks = {
    ...
    addSchedule: useCallback(() => {
      ...
      dispatch(parserActions.load(file, year, semester, status, department));
  ...
  useEffect(() => {
    if (select.parserWaiting && result == null) {
      toast.info('Идет парсинг расписания')
    }
    if (select.parser == false) {
      toast.error('Произошла ошибка при парсинге расписания')
    }
    if (select.parser) {
      toast.done('Парсинг прошел успешно')
    }
  }, [select.parser])
